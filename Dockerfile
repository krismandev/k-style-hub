...
COPY . .
RUN go get -d -v
RUN go build -ldflags="-w -s -X 'main.version=$VER' -X 'main.builddate=$BUILDDATE'" -o /go/bin/kstlehub
...
COPY --chown=app --from=builder /go/bin/kstlehub /app/kstlehub.app
COPY --chown=app config/config.yml /app/config/config.yml
COPY . .
CMD /app/kstlehub.app
